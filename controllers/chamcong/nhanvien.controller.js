...
const themNhanVien = async (req, res) => {
  try {
    const { ma_nhan_vien, ten_nhan_vien, bo_phan, chuc_vu, email, so_dien_thoai } = req.body;

    if (!ma_nhan_vien || !ten_nhan_vien || !bo_phan) {
      return res.status(400).json({ message: "Thiếu thông tin bắt buộc (mã, tên, bộ phận)" });
    }

    const existing = await NhanVien.findOne({ ma_nhan_vien: ma_nhan_vien.toUpperCase() });
    if (existing) {
      return res.status(409).json({ message: `Mã nhân viên ${ma_nhan_vien} đã tồn tại` });
    }

    const nhanVien = await NhanVien.create({
      ma_nhan_vien: ma_nhan_vien.toUpperCase(),
      ten_nhan_vien, bo_phan, chuc_vu, email, so_dien_thoai,
    });

    res.status(201).json({ message: "Thêm nhân viên thành công", data: nhanVien });
  } catch (error) {
...
const themNhieuNhanVien = async (req, res) => {
  try {
    ...
    if (!Array.isArray(data) || data.length === 0) {
      return res.status(400).json({ message: "Dữ liệu import phải là mảng và không được rỗng" });
    }

    const MAX_IMPORT = 500;
    if (data.length > MAX_IMPORT) {
      return res.status(400).json({ message: `Chỉ được import tối đa ${MAX_IMPORT} nhân viên mỗi lần` });
    }
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const { ma_nhan_vien, ten_nhan_vien, bo_phan, chuc_vu, email, so_dien_thoai } = row;
      ...
      try {
        ...
        if (existing) {
          results.skipped.push({ row: rowIndex, ma_nhan_vien: maNormalized, reason: "Mã đã tồn tại" });
          continue;
        }
        ...
        results.success.push({ row: rowIndex, ma_nhan_vien: maNormalized, id: created._id });
      } catch (rowError) {
        results.failed.push({ row: rowIndex, ma_nhan_vien: maNormalized, reason: rowError.message });
      }
    }
...
const traCuuNhanVien = async (req, res) => {
  try {
    ...
    const nhanVien = await NhanVien.findOne({ ma_nhan_vien: ma_nhan_vien.toUpperCase() });
    ...
    if (!nhanVien.active) {
      return res.status(403).json({ message: "Nhân viên này đã bị khóa, không thể chấm công" });
    }
...
const capNhatNhanVien = async (req, res) => {
  try {
    ...
    const nhanVien = await NhanVien.findByIdAndUpdate(id, update, { new: true });
...
module.exports = {
  themNhanVien, themNhieuNhanVien,
  traCuuNhanVien, getDanhSach,
  capNhatNhanVien, toggleActive, xoaNhanVien,
};